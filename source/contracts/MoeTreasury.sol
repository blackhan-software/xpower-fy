 ...
 */
contract MoeTreasury is
    ReentrancyGuardTransient,
    MoeTreasurySupervised,
    Ownable,
    Banq
{
    ...
    constructor(
        ...
        Banq(sovLink, 1440 * 10 ** APower(sovLink).decimals())
    {
        _moe = XPower(moeLink);
        _sov = APower(sovLink);
    ...
    function claimBatch(
        ...
        require(Array.unique(nftIds), NonUnique(nftIds));
        uint256[] memory moe_amount = claimableBatch(account, nftIds);
        uint256[] memory sov_minted = new uint256[](nftIds.length);
        for (uint256 i = 0; i < nftIds.length; i++) {
        ...
        emit ClaimBatch(account, nftIds, sov_minted);
    }
    ...
    /** Thrown on non-unique (or sorted) NFT IDs. */
    error NonUnique(uint256[] nft_ids);
    /** Thrown on invalid claim. */
